...
let
  ...
  vendorCfg = cfg.monitoring.telegraf.inputs.snmp.vendors.reddoxx;
  ...
  config = {
    services.telegraf.extraConfig.inputs.snmp = lib.mkIf (snmpCfg.enable && vendorCfg.endpoints.self.enable) (
      map (agent: {
        ...
        timeout = "20s";
        version = 3;
        sec_level = "${vendorCfg.credentials.security.level}";
        sec_name = "${vendorCfg.credentials.security.username}";
        auth_protocol = "${vendorCfg.credentials.authentication.protocol}";
        auth_password = "${vendorCfg.credentials.authentication.password}";
        priv_protocol = "${vendorCfg.credentials.privacy.protocol}";
        priv_password = "${vendorCfg.credentials.privacy.password}";
        retries = 5;
        field = [
      ...
      }) vendorCfg.endpoints.self.agents
    );
  };
}
